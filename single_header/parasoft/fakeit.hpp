...
#include <vector>
#include <stdexcept>
#if defined (__GNUG__) || _MSC_VER >= 1900 || (__ICCARM__)
...
#if defined (__GNUG__) || (__ICCARM__)
...
namespace fakeit {
    ...
    inline RepeatedSequence operator*(const Sequence &s, int times) {
        if (times <= 0) {
#ifdef PARASOFT_CPPTEST
        	CPPTEST_FAIL("Invalid argument: times");
        	CPPTEST_BREAK();
#else
...
namespace fakeit {
    ...
    public:
        ...
        getDestructorOffset() {
#ifdef PARASOFT_CPPTEST
            CPPTEST_FAIL("Can't mock the destructor. No virtual destructor was found");
        	CPPTEST_BREAK();
#else
            throw NoVirtualDtor();
#endif
...
namespace fakeit {
    ...
    private:
        ...
        void checkMultipleInheritance() {
            ...
            if (delta > 0) {
#ifdef PARASOFT_CPPTEST
            	CPPTEST_FAIL("multiple inheritance is not supported");
            	CPPTEST_BREAK();
#else
    ...
    struct DefaultValue<C, typename std::enable_if<!is_constructible_type<C>::value>::type> {
        static C &value() {
        	...
        	static_assert(std::is_reference<C>::value,
        			"Type C is not default constructible. Could not instantiate a default return value");
...
namespace fakeit {
    ...
    class RecordedMethodBody : public MethodInvocationHandler<R, arglist...>, public ActualInvocationsSource, public ActualInvocationsContainer {
        ...
        struct MatchedInvocationHandler : ActualInvocationHandler<R, arglist...> {
            ...
            {
                ActualInvocationHandler<R, arglist...> *invocationHandler = dynamic_cast<ActualInvocationHandler<R, arglist...> *>(_invocationHandler.get());
...
namespace fakeit {
    ...
    _Time(unsigned long long n) {
        if (n != 1) {
#ifdef PARASOFT_CPPTEST
        	CPPTEST_FAIL("Only 1_Time is supported. Use X_Times (with s) if X is bigger than 1");
        	CPPTEST_BREAK();
#else
...
namespace fakeit {
    ...
    struct MethodStubbingProgress {
        ...
        Return(const R &r) {
            auto method = [r](const arglist &...) -> R { return r; };
            return Do(method);
        }
...
namespace fakeit {
    ...
    struct ActionSequence : ActualInvocationHandler<R,arglist...> {
        ...
        virtual R handleMethodInvocation(ArgumentsTuple<arglist...> & args) override
        {
            ...
            Action<R, arglist...> *action = dynamic_cast<Action<R, arglist...> *>(destructablePtr.get());
    ...
    private:
        ...
        struct NoMoreRecordedAction : Action<R, arglist...> {
            ...
            virtual R invoke(const ArgumentsTuple<arglist...> &) override {
#ifdef PARASOFT_CPPTEST
            	CPPTEST_FAIL("Unable to handle invocation for Mocked method - no more recorded actions for method.");
            	CPPTEST_BREAK();
#else
...
namespace fakeit {
    ...
    public:
	  ...
	  : _instanceOwner(nullptr),
	    _proxy{obj},
	    _fakeit(fakeit) {}

        MockImpl(FakeitContext &fakeit)
	  : _instanceOwner(createFakeInstance()),
	    _proxy(*(reinterpret_cast<C *>(_instanceOwner.get()))),
	    _fakeit(fakeit) {
    ...
    private:
        ...
        static FakeObject<C, baseclasses...> *createFakeInstance() {
...
namespace fakeit {
    ...
    class CalledEventHandler : public VerificationEventHandler {
...
namespace fakeit {
    ...
    public:
        ...
        Terminator Exactly(const int times) {
            if (times < 0) {
#ifdef PARASOFT_CPPTEST
            	CPPTEST_FAIL( cpptestFormat( "bad argument times: %d", times ) );
            	CPPTEST_BREAK();
#else
