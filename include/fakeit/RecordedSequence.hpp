...
/**
 * Represents a list of recorded actions created by one stubbing list:
 * When(Method(mock,foo)).Return(1).Throw(e).AlwaysReturn(2);
 *                       ^------Recorded------Sequence-----^
 */
template<typename R, typename ... arglist>
struct RecordedSequence: public MethodInvocationHandler<R, arglist...> {

	RecordedSequence(Method & method) :
			_method(method) {
		clear();
	}
