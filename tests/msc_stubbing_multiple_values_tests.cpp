...
struct MultipleStubbing: tpunit::TestFixture {
	...
	void stub_multiple_return_values_with_quantifier() {
		...
		When(mock[&SomeInterface::func]).Return(Times<2>::of(1), Times<2>::of(2));
		When(mock[&SomeInterface::proc]).Return(Times<2>::Void());
	...
	void stub_multiple_return_values_with_mixed_values_and_quantifiers() {
		Mock<SomeInterface> mock;
		When(mock[&SomeInterface::func]).Return(0, Times<2>::of(1), 2);
		SomeInterface &i = mock.get();
	...
	void stub_multiple_throws_with_quantifier() {
		Mock<SomeInterface> mock;
		When(mock[&SomeInterface::func]).Throw(Times<2>::of(std::string("1")),Times<1>::of(1));
		When(mock[&SomeInterface::proc]).Throw(Times<2>::of(std::string("1")),Times<1>::of(1));
		SomeInterface &i = mock.get();
//
	...
	void stub_multiple_throws_and_returns() {
		Mock<SomeInterface> mock;
		When(mock[&SomeInterface::func]).Throw(std::string("1"),Times<1>::of(1)).Return(1,Times<1>::of(2));
		When(mock[&SomeInterface::proc]).Throw(std::string("1"),Times<1>::of(1)).Return(Times<2>::Void());
		SomeInterface &i = mock.get();
//
		ASSERT_THROW(i.func(1), std::string);
		ASSERT_THROW(i.func(1), int);
	...
	void stub_multiple_do_with_list_of_mixed_quatifiers_and_values() {
		Mock<SomeInterface> mock;
		When(mock[&SomeInterface::func]).Do(Times<1>::of([](...){return 1;}), [](...){return 2;});
		When(mock[&SomeInterface::proc]).Do(Times<1>::of([](...){}), [](...){});
		SomeInterface &i = mock.get();
